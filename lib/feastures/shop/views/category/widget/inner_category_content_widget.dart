...
class InnerCategoryContentWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      //appBar: TAppBar(title: Text(category.name, style: Theme.of(context).textTheme.headlineMedium!.apply(color: Colors.black)), showBackArrow: true),
      appBar: PreferredSize(
        preferredSize: const Size.fromHeight(kToolbarHeight),
        child: Container(
          color: TColors.primary, // <-- Add your desired background color here
          child: TAppBar(showBackArrow: true, title: Text(category.name, style: Theme.of(context).textTheme.headlineMedium!.apply(color: dark ? TColors.dark : TColors.light))),
        ),
      ),
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            Center(child: Text('Shop By SubCategories:', style: GoogleFonts.quicksand(textStyle: Theme.of(context).textTheme.bodyMedium, fontSize: 15, fontWeight: FontWeight.bold, letterSpacing: 1.7, color: Colors.grey))),
            const SizedBox(height: TSizes.spaceBtwItems),
